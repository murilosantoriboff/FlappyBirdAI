...
class Passaro:
    ...
    def __init__(self, x, y):
        ...
        self.imagem_passsaro = self.IMGS[1]
    ...
    def desenhar(self, tela):
        # Defenir qual imagem do passaro usar
    ...
    def get_mask(self):
        return pygame.mask.from_surface(self.imagem_passsaro)
...
def main(genomas, config):
    ...
    while rodando:
        ...
        for i, passa in enumerate(passaro):
            passa.mover()
            lista_genomas[i].fitness += 0.1
            output = redes[i].activate((passa.y, abs(passa.y - canos[indice_cano].altura), abs(passa.y - canos[indice_cano].posicao_base)))
            if output[0] > 0.5:
                passa.pular()
        chao.mover_chao()
        ...
        for cano in canos:
            for i, passa in enumerate(passaro):
                if cano.colidir(passa):
                    passaro.pop(i)
                    if ai_jogando:
                        lista_genomas[i].fitness -= 1
                        lista_genomas.pop(i)
                        redes.pop(i)
                if not cano.passou and passa.x > cano.x:
                    cano.passou = True
                    adicionar_cano = True
            cano.mover_cano()
        ...
        for i, passa in enumerate(passaro):
            if (passa.y + passa.imagem_passsaro.get_height()) > chao.y or passa.y < 0:
                passaro.pop(i)
                if ai_jogando:
                    lista_genomas.pop(i)
                    redes.pop(i)
...
if __name__ == '__main__':
    ...
    caminho = os.path.join(cam, 'NEAT.txt')
    rodar(caminho)